...
cpgFile=$tmpdir2/$cpg.txt
echo $cpg > $cpgFile
covFile=$tmpdir2/Covariates.$cpg.$rank.$tissue.$id.fwd.txt;
methFile=$tmpdir2/"$tissue"_"$id".gz
outFileTmp=$tmpdir/tmpFiles/$tissue.$id.$cpg.$rank.$mode.txt
...
if [[ $mode = "regular" ]]; then
        ...
        cut -f 1 $covFile | tail -n 1 > $cpgFile.SNP.exclude
        if [ $nchr -lt 3 ]; then
                fastQTL --vcf $vcfFile --bed $methFile --cov $covFile --out $outFileTmp --chunk 1 1 --include-samples $sampleFile --maf-threshold 0.01 -W 500000 --include-phenotypes $cpgFile --exclude-sites $cpgFile.SNP.exclude
                gzip -f $outFileTmp;
                ls $outFileTmp.gz;
        else
                g=$(cat $cpgFile)
                zcat $methFile | head -n 1 > $methFile.$g
                zgrep $g $methFile >> $methFile.$g
                bgzip -f $methFile.$g
                tabix -p bed $methFile.$g.gz
                fastQTL --vcf $vcfFile --bed $methFile.$g.gz  --cov $covFile --out $outFileTmp --chunk 1 1 --include-samples $sampleFile --maf-threshold 0.01 -W 500000 --include-phenotypes $cpgFile --exclude-sites $cpgFile.SNP.exclude
                gzip -f $outFileTmp;
                rm $methFile.$g.gz $methFile.$g.gz.tbi
        fi
fi
if [[ $mode = "conditional" ]]; then
        ...
        if [ $nchr -lt 3 ]; then
                echo "Chunk $id of $tissue"
                fastQTL --vcf $vcfFile --bed $methFile --cov $covFile --out $outFileTmp --chunk 1 1 --include-samples $sampleFile --maf-threshold 0.01 -W 500000 --include-phenotypes $cpgFile --map $thresholds --include-phenotypes $thresholds_phenotypes 
		gzip -f $outFileTmp;
        else
                outFileTmp=$tmpdir/tmpFiles/$tissue.$id.$mode.1.txt
                fastQTL --vcf $vcfFile --bed $methFile --cov $covFile --out $outFileTmp --chunk 1 2 --include-samples $sampleFile --maf-threshold 0.01 -W 500000 --include-phenotypes $cpgFile --map $thresholds --include-phenotypes $thresholds_phenotypes
                outFileTmp=$tmpdir/tmpFiles/$tissue.$id.$mode.2.txt
                fastQTL --vcf $vcfFile --bed $methFile --cov $covFile --out $outFileTmp --chunk 2 2 --include-samples $sampleFile --maf-threshold 0.01 -W 500000 --include-phenotypes $cpgFile --map $thresholds --include-phenotypes $thresholds_phenotypes
...
if [[ $mode = "interaction" ]]; then
	...
	interactionFile=$wdir/data/Covariates/interaction_Covariates/$interaction/$tissue.$interaction.txt
		...
		outFileTopTmp=$tmpdir/tmpFiles/$tissue.$id.$mode.$interaction.topSNP.txt
                echo "Chunk $id of $tissue"
                fastQTL --vcf $vcfFile --bed $methFile --cov $covFile --out $outFileTmp --chunk 1 1 --include-samples $sampleFile --maf-threshold 0.01 -W 500000 --include-phenotypes $cpgFile --interaction $interactionFile --report-best-only 
		...
		outFileTopTmp=$tmpdir/tmpFiles/$tissue.$id.$mode.$interaction.topSNP.1.txt
                fastQTL --vcf $vcfFile --bed $methFile --cov $covFile --out $outFileTmp --chunk 1 2 --include-samples $sampleFile --maf-threshold 0.01 -W 500000 --include-phenotypes $cpgFile --interaction $interactionFile --report-best-only
		cat $outFileTmp | awk ' NR==1 { gene=$1;genepval[gene]=$7;line=$0 } { if ($1 == gene) { if ($7 < genepval[gene]) { genepval[gene]=$7; line=$0 } } else { print line; gene=$1; genepval[gene]=$7; line=$0 } } END { print line }' > $outFileTopTmp
		gzip -f $outFileTmp;
		...
		outFileTmp=$tmpdir/tmpFiles/$tissue.$id.$mode.$interaction.2.txt
                ...
                fastQTL --vcf $vcfFile --bed $methFile --cov $covFile --out $outFileTmp --chunk 2 2 --include-samples $sampleFile --maf-threshold 0.01 -W 500000 --include-phenotypes $cpgFile --interaction $interactionFile --report-best-only
		cat $outFileTmp | awk ' NR==1 { gene=$1;genepval[gene]=$7;line=$0 } { if ($1 == gene) { if ($7 < genepval[gene]) { genepval[gene]=$7; line=$0 } } else { print line; gene=$1; genepval[gene]=$7; line=$0 } } END { print line }' > $outFileTopTmp
		gzip -f $outFileTmp;
        fi
fi
