...
save(file='mQTLs/mQTLs.FastQTLSumStats.mash.simple.vhat.Robj',list=c('data.random','data.strong'))

# Compute prior matrices
# FLASH prior covariances
U.flash = cov_flash(mash_data, non_singleton = TRUE, save_model = "mash_output/mQTLs.FLASH.model.rds")
save(file="mash_output/mQTLs.FLASH.Robj",U.flash)
quit(save='no')
