...
file=paste0(tmpdir,'/results/',args$tissue,'/mqtls/NonPermutedResults/tmpFiles/',args$tissue,'.indep_eQTLs.',args$mode,'.',args$gwas,'.',args$gwasp,'.',args$gwasid,'.all_eQTLs.txt')
file_genes=paste0(tmpdir,'/results/',args$tissue,'/mqtls/NonPermutedResults/tmpFiles/',args$tissue,'.indep_eQTLs.',args$mode,'.',args$gwas,'.',args$gwasp,'.',args$gwasid,'.genes.all_eQTLs.txt')
print(file)
statsall=read.table(file,header=F)
genes=as.character(read.table(file_genes,header=F)[,1])
...
statsall$gene_id=as.character(statsall$gene_id)
...
for (gene in genes) {
	...
	# Impute GWAS and eQTL effect size
...
#results is an annotated version of the input data containing log Approximate Bayes Factors and intermediate calculations, and the posterior probability SNP.PP.H4 of the SNP being causal for the shared signal
	...
	file=paste0(wdir,'/results/',args$tissue,'/',args$gwas,'/colocsummary/',args$tissue,'.indep_eQTLs.',args$mode,'.',args$gwas,'.',args$gwasp,'.',args$gwasid,'.standard_GWAS.all_eQTLs.LD.txt')
	print(file)
