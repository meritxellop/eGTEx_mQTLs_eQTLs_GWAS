...
for tissue in $(cat /gpfs/data/gtex-group/mpavia/methylation/data/support_files/tissues.txt); do
	...
	eqtlFile=/scratch/mpavia/colocalization/results/$tissue/finemapping/dap_vcf_dir/fastenloc.$tissue.mqtl.annotation.vcf.gz
	# Generate pre-computed mQTL annotations following guidelines of fastenloc github repository: https://github.com/xqwen/fastenloc
	...
	for gwas in $(cat $wdir/data/GWASes.87.txt); do
		if [[ -f /scratch/mpavia/colocalization/results/$tissue/fastenloc/$tissue.$gwas.all.enloc.enrich.out ]]; then continue; fi
...
#PBS -l mem=10GB
#PBS -d $wdir
#PBS -o $tmpdir/logs/run_fastenloc_"$tissue"_"$gwas"_\$PBS_JOBID.out
#PBS -e $tmpdir/logs/run_fastenloc_"$tissue"_"$gwas"_\$PBS_JOBID.err
date
...
fastenloc -eqtl $eqtlFile -gwas \$gwasFile -t $tissue -thread 1 --all -prefix /scratch/mpavia/colocalization/results/$tissue/fastenloc/$tissue.$gwas.all

date
" > $tmpdir/logs/run_fastenloc_"$tissue"_"$gwas".qsub
qsub $tmpdir/logs/run_fastenloc_"$tissue"_"$gwas".qsub;
#bash $tmpdir/logs/run_fastenloc_"$tissue"_"$gwas".qsub;
	done;
done
