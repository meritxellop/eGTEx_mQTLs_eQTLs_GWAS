...
file=paste0(tmpdir,'/results/',args$tissue,'/mqtls/NonPermutedResults/tmpFiles/',args$tissue,'.',args$mode,'.',args$gwas,'.',args$gwasp,'.',args$gwasid,'.all_mQTLs.txt')
file_cpgs=paste0(tmpdir,'/results/',args$tissue,'/mqtls/NonPermutedResults/tmpFiles/',args$tissue,'.',args$mode,'.',args$gwas,'.',args$gwasp,'.',args$gwasid,'.cpgs.all_mQTLs.txt')
print(file)
statsall=read.table(file,header=F)
cpgs=read.table(file_cpgs,header=F)[,1]
...
#results is an annotated version of the input data containing log Approximate Bayes Factors and intermediate calculations, and the posterior probability SNP.PP.H4 of the SNP being causal for the shared signal
	...
	my.res.all.top=my.res.all.top[-1,]
        ...
        file=paste0(wdir,'/results/',args$tissue,'/',args$gwas,'/colocsummary/',args$tissue,'.standard_mQTLs.',args$mode,'.',args$gwas,'.',args$gwasp,'.',args$gwasid,'.standard_GWAS.all_mQTLs.LD.txt.tmp')
